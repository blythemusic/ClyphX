"""
# Copyright (C) 2016 Stray <stray411@hotmail.com>
#
...
class ClyphXArsenalActions(ControlSurfaceComponent):
        ...
        if script['scl']:
            ...
            if len(spec) == 1:
                ...
                if spec[1] == 'INKEY':
                    ...
                    toggle_property(scl, '_sequent_layout', spec[2:])
                elif spec[1] == 'ROOT':
                    adjust_property(scl._tonics, '_page_index', 0,
                                    scl._tonics.num_pages - 1, spec[2:], 'set_page_index',
                                    NOTE_NAMES)
                elif spec[1] == 'TYPE':
    ...
    def _capture_scale_settings(self, script, xclip, ident):
        """ Captures the current scale type, tonic, in key state and layout and adds them
        to the given xclip's name. """
        if type(xclip) is Live.Clip.Clip:
            comp = script['scl']
            xclip.name = '%s %s SCL %s %s %s %s' % (ident, script['top'].script_name,
    ...
    def _recall_scale_settings(self, comp, spec):
        ...
        comp._sequent_layout = spec[4].strip() == 'TRUE'
